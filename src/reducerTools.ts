...
export const autoReducerFactory = <AT>(actionTypes : AT) => <AD extends { [T in keyof AT] : AD[T] }>() => {
  ...
  const reducers = {} as { [T in keyof AT] : {
    exampleState : AD[T],
    makeActionReducer : (defaultState : AD[T]) => (state : AD[T] | undefined, action : AnyAction) => AD[T],
    makePropertyReducer : <P extends keyof AD[T]>(property : P, defaultState : AD[T][P]) => (state : AD[T][P] | undefined, action : AnyAction) => AD[T][P],
  } };
